...
# Database setup (Render will inject DATABASE_URL, fallback to local SQLite)
...
class UserDB(Base):
    ...
    monthly_budget = Column(Float, default=0.0)  # 👈 Each user can set a budget
...
def login(user: UserLogin, db: Session = Depends(get_db)):
    ...
    return {"message": "Login successful", "user_id": db_user.id, "monthly_budget": db_user.monthly_budget}
